...
export class PokeAPIService {
  ...
  setInfosOnPokemon(pokemon: Pokemon): Promise<void> {
    let pokemonName = pokemon.name.toLocaleLowerCase() === "jellicent" ? "jellicent-male" : pokemon.name;
  ...
  getMovesType(moves: string[]): Record<string, string> {
    ...
    moves.forEach(move => {
      const hiddenPowerMatch = move.match(
        /^Hidden(?:\s+|-)?Power(?:\s+\[([^\]]+)\]|\s+([A-Za-z]+)|-([A-Za-z]+))?$/i
      );

      if (hiddenPowerMatch) {
        const type = (hiddenPowerMatch[1] ?? hiddenPowerMatch[2]).toLowerCase();
        movesAndTypes[move] = type;
      } else {
        this.http
          .get(`https://pokeapi.co/api/v2/move/${move.replace(/\s+/g, '-').toLowerCase()}`)
          .subscribe((response: any) => {
            movesAndTypes[move] = response?.type?.name;
          });
      }
    });
